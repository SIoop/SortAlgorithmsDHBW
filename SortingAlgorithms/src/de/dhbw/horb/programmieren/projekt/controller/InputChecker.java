...
public class InputChecker {
	...
	public InputChecker(InputMode mode, String lowerLimit, String upperLimit, String amount, String manualInput,
			String fileInput, String threads, String delay, String runs) {
	...
	public void check() throws IncorrectInputException {
		...
		if (mode == InputMode.FILE) {
			...
			if (fileInput.equals(""))
				exceptionMessage = exceptionMessage.concat("Relevante Felder dürfen nicht leer sein!\n");
			...
			for (int i = 0; i < manualInput.length(); i++) {
				...
				if (!Character.isDigit(manualInput.charAt(i)) && manualInput.charAt(i) != ',') {
					exceptionMessage = exceptionMessage
							.concat("Die manuelle Eingabe darf nur aus Zahlen und Kommas bestehen!\n");
					break;
				}
			}
			if (manualInput.equals(""))
				exceptionMessage = exceptionMessage.concat("Relevante Felder dürfen nicht leer sein!\n");
		} else {

			// Check Lower Limit: Correct Format + <= Upper Limit
			try {
				int low = Integer.parseInt(lowerLimit);
				int high = Integer.parseInt(upperLimit);
				if (low > high)
					exceptionMessage = exceptionMessage
							.concat("Untere Grenze darf nicht größer sein als obere Grenze!\n");
			} catch (NumberFormatException e) {
				exceptionMessage = exceptionMessage
						.concat("Untere Grenze für Zufallszahlen muss eine korrekte Zahl sein!\n");
			}

			// Check Upper Limit: Correct Format + < MaxValue
			try {
				int high = Integer.parseInt(upperLimit);
				if (high == Integer.MAX_VALUE) {
					exceptionMessage = exceptionMessage.concat("Obere Grenze muss kleiner als 2147483647 sein!\n");
				}
			} catch (NumberFormatException e) {
				exceptionMessage = exceptionMessage
						.concat("Obere Grenze für Zufallszahlen muss eine korrekte Zahl sein!\n");
			}
			...
			try {
				...
				if (am * 8 > Runtime.getRuntime().freeMemory()) {
					exceptionMessage = exceptionMessage.concat("Zu wenig Speicher (Verfügbar: "
							+ Runtime.getRuntime().freeMemory() + ", benötigt: " + am * 4 + ")!\n");
				}
			} catch (NumberFormatException e) {
		...
		if (threads.equals("") || delay.equals("") || runs.equals(""))
			exceptionMessage = exceptionMessage.concat("Relevante Felder dürfen nicht leer sein!\n");
